...
class ChunkingPlugin extends ServerPlugin {
	 ...
	 * @param string $destination destination path
	 */
	function afterMove($sourcePath, $destination) {
		if (!$this->sourceNode instanceof FutureFile) {
			// skip handling as the source is not a chunked FutureFile
			return;
		}
		...
		if ($destinationNode instanceof File) {
			$destinationNode->touch($mTime);
			$this->server->httpResponse->addHeader('X-OC-MTime', 'accepted');
		}
	}
	...
	private function verifySize() {
		$expectedSize = $this->server->httpRequest->getHeader('X-OC-Total-Size');
